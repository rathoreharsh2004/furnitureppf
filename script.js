(() => {
  ...
  const hidePreloader = () => {
    ...
    // Remove it from the page after the fade so it can never block clicks/scroll.
    setTimeout(() => preloader.remove(), 700);
  };

  // Do not wait for every remote image. The page can load independently.
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => setTimeout(hidePreloader, 450), { once: true });
  } else {
    setTimeout(hidePreloader, 450);
  }

  // Hard fallback in case a browser/plugin delays DOM events.
